...
class ForageServiceTest {
    ...
    void ShouldNotAddDuplicate() throws DataException {
        ArrayList<Forage> forages = new ArrayList<>();
        ...
        Result<Forage> result = service.add(forage);
        assertEquals("[This is a duplicate forage!]", result.getErrorMessages().toString());
    }

    @Test
    void shouldFindReportForSums() {
        final LocalDate date = LocalDate.of(2020,6,26);
        List<Forage> list = service.findByDate(date);
        BigDecimal[] array = service.findReportForSums(list);
        long result = array[0].longValue();
        long expected = (long) 12.487500;
