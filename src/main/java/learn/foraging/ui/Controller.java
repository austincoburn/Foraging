...
public class Controller {
        ...
        do {
            ...
            switch (option) {
                ...
                case REPORT_KG_PER_ITEM:
                    viewReportOfKilograms();
                    break;
                case REPORT_CATEGORY_VALUE:
                    viewReportOfCategorySums();
                    break;
                case VIEW_FORAGERS:
                    viewForagers();
                    break;
    ...
    private void viewReportOfKilograms() {
        LocalDate date = view.getForageDateForReport();
        List<Forage> forages = forageService.findByDate(date);
        view.displayKilogramReport(forages);
    }

    private void viewReportOfCategorySums() {
        LocalDate date = view.getForageDateForReport();
        List<Forage> forages = forageService.findByDate(date);
        BigDecimal[] result = forageService.findReportForSums(forages);
        view.displayReportForSumsCategorically(result, forages);
    }

    private void viewForagers() {
        List<Forager> foragers = foragerService.findAllForagers();
