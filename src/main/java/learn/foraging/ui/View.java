...
public class View {
    ...
    public LocalDate getForageDateForReport() {
        return io.readLocalDate("Select a date [MM/dd/yyyy]: ");
    }
    ...
    public Forager makeForager() {
        ...
        forager.setState(io.readStateString("State [ex: WA, CO, FL]: "));
        return forager;
    }
    ...
    public void displayKilogramReport(List<Forage> forages) {
        ...
        for(Forage forage : forages) {
            io.printf("Item Id: %s , Item Name: %s , Kilograms: %s%n", forage.getItem().getId(), forage.getItem().getName(), forage.getKilograms());
        }
    }

    public void displayReportForSumsCategorically(BigDecimal[] result, List<Forage> forages) {
        displayHeader(MainMenuOption.REPORT_CATEGORY_VALUE.getMessage());
        if (forages == null || forages.isEmpty()) {
            io.println("No forages found.");
            return;
        }
        System.out.printf("Category: %s, Total value: $%.2f%nCategory: %s, Total value: $%.2f%nCategory: %s, Total value: $%.2f%nCategory: %s, Total value: $%.2f%n",
                Category.EDIBLE, result[0], Category.INEDIBLE, result[1],
                                Category.MEDICINAL, result[2], Category.POISONOUS, result[3]);
    }
    ...
    public void displayForagers(List<Forager> foragers) {
        ...
        for(Forager forager : foragers) {
            io.printf("Id: %s, First Name: %s, Last Name: %s, State: %s%n", forager.getId(), forager.getFirstName(), forager.getLastName(), forager.getState());
        }
        enterToContinue();
    }
