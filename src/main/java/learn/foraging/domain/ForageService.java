...
public class ForageService {
    ...
    public BigDecimal[] findReportForSums(List<Forage> forages) {
        BigDecimal sumEdible = new BigDecimal(0);
        BigDecimal sumInedible = new BigDecimal(0);
        BigDecimal sumMedicinal = new BigDecimal(0);
        BigDecimal sumPoisonous = new BigDecimal(0);


        sumEdible = forages.stream().filter(a -> a.getItem().getCategory() == Category.EDIBLE).map(Forage::getValue).reduce(sumEdible, BigDecimal::add);
        sumInedible = forages.stream().filter(a -> a.getItem().getCategory() == Category.INEDIBLE).map(Forage::getValue).reduce(sumInedible, BigDecimal::add);
        sumMedicinal = forages.stream().filter(a -> a.getItem().getCategory() == Category.MEDICINAL).map(Forage::getValue).reduce(sumMedicinal, BigDecimal::add);
        sumPoisonous = forages.stream().filter(a -> a.getItem().getCategory() == Category.POISONOUS).map(Forage::getValue).reduce(sumPoisonous, BigDecimal::add);

        BigDecimal[] array = {sumEdible, sumInedible, sumMedicinal, sumPoisonous};
        return array;
    }
    ...
    private boolean findByDateForagerItem(LocalDate date, Forager forager, Item item) {
        List<Forage> filteredForages = findByDate(date);
        boolean isDuplicate = filteredForages.stream().anyMatch(a -> a.getForager().getId().equalsIgnoreCase(forager.getId()))
                && filteredForages.stream().anyMatch(b -> b.getItem().getId() == item.getId());
        if(isDuplicate == true) {
            return true;
        }
        return false;
    ...
    private Result<Forage> validate(Forage forage) {
        ...
        if(findByDateForagerItem(forage.getDate(), forage.getForager(), forage.getItem())) {
